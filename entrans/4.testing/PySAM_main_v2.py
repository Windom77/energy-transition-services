...
def run_simulation(input_json: Optional[str] = None) -> SimulationResults:
    """SIMPLIFIED: Main entry point without complex progress callbacks - ALL FUNCTIONALITY PRESERVED"""

    try:
        logging.info("🚀 Starting Simplified PySAM simulation with complete functionality...")
        ...
        # Create configuration objects (preserved functionality)
        ...
        logging.info(f"🔋 Battery system: {'Enabled' if battery_params.enabled else 'Disabled'}")
        logging.info(f"🌿 LREC system: {'Enabled' if lrec_config.enabled else 'Disabled'}")

        # Validate configurations
        battery_warnings = BatteryDetector.validate_battery_config(battery_params)
        for warning in battery_warnings:
            logging.warning(f"🔋 {warning}")

        lrec_warnings = LRECDetector.validate_lrec_config(lrec_config)
        for warning in lrec_warnings:
            logging.warning(f"🌿 {warning}")

        # Initialize error handler
        error_handler = ErrorHandler(strict_mode=False)

        # Initialize PySAM modules (simplified but complete)
        logging.info("🔧 Initializing PySAM modules...")
        ...
        # Initialize modules with complete error handling
        for i, mod_name in enumerate(module_order):
            try:
                ...
                if i == 0:
                    ...
                    module_instance = pysam_module.from_existing(modules[prev_mod_name])
                    module_instance.assign(pysam_module.wrap(mod_data).export())

                modules[mod_name] = module_instance
                logging.info(f"✅ Initialized: {mod_name}")

            except Exception as e:
                if not error_handler.handle_module_error(mod_name, e):
                    continue
                else:
                    raise

        logging.info(f"🔧 Initialized {len(modules)} modules")

        # Assign weather file (preserved functionality)
        ...
        if weather_file and modules:
            ...
            if hasattr(first_module, "SolarResource"):
                ...
                logging.info(f"☀️ Weather file assigned: {Path(weather_file).name}")

        # Execute modules with complete error handling
        logging.info("⚡ Executing PySAM modules...")
        ...
        for i, mod_name in enumerate(module_order):
            if mod_name in modules:
                try:
                    ...
                    logging.info(f"✅ Executed: {mod_name}")
                except Exception as e:
                    if not error_handler.handle_module_error(mod_name, e):
                        continue
                    else:
        ...
        logging.info(f"⚡ Module execution completed: {successful_executions}/{len(modules)} successful")

        if successful_executions == 0:
            raise RuntimeError("No modules executed successfully")

        # Collect results (preserved complete functionality)
        ...
        # Process FCAS (preserved complete functionality)
        fcas_results = {}
        if battery_params.enabled:
            try:
                logging.info("🇦🇺 Processing FCAS...")
                fcas_processor = FCASProcessor(config_dict, battery_params, error_handler)
                fcas_results = fcas_processor.process_fcas(modules)

                if fcas_results and 'total_ancillary_revenue' in fcas_results:
                    total_revenue = fcas_results['total_ancillary_revenue']
                    logging.info(f"✅ FCAS Revenue: ${total_revenue:,.0f}")

            except Exception as e:
                logging.warning(f"FCAS processing failed: {e}")
                fcas_results = {}

        # Process LREC (preserved complete functionality)
        lrec_results = {}
        try:
            logging.info("🌿 Processing LREC...")
            ...
            if lrec_results and 'total_lrec_revenue' in lrec_results:
                total_revenue = lrec_results['total_lrec_revenue']
                logging.info(f"✅ LREC Revenue: ${total_revenue:,.0f}")

        except Exception as e:
            logging.warning(f"LREC processing failed: {e}")
            lrec_results = {}

        # Integrate all results (preserved complete functionality)
        logging.info("🔗 Integrating results...")
        ...
        # Export results (preserved complete functionality)
        logging.info("💾 Exporting results...")
        results_exporter = ResultsExporter(analysis_period)
        metadata = results_exporter.export_all_results(results)

        # Log completion summary (preserved)
        logging.info("✅ Simplified PySAM simulation completed successfully")
        logging.info(f"📊 Results Summary:")
        logging.info(f"   Scalar Results: {len(scalar_results)}")
        logging.info(f"   Time Series: {len(time_series)}")
        logging.info(f"   FCAS Revenue: ${fcas_results.get('total_ancillary_revenue', 0):,.0f}")
        logging.info(f"   LREC Revenue: ${lrec_results.get('total_lrec_revenue', 0):,.0f}")
        logging.info(f"   Modules Executed: {successful_executions}/{len(module_order)}")

        error_summary = error_handler.get_summary()
        if error_summary['total_warnings'] > 0:
            logging.info(f"   Warnings: {error_summary['total_warnings']}")

        return results

    except Exception as e:
        logging.error(f"💥 Simplified PySAM simulation failed: {e}")
        import traceback
...
if __name__ == "__main__":
    try:
        setup_logging()
        logging.info("🚀 Starting Simplified Complete PySAM Simulation with LREC Support...")
        ...
        if results.scalar_results:
            logging.info("✅ Simplified complete simulation completed successfully")
            logging.info("📋 All original functionality preserved except progress tracking")
        else:
            logging.warning("⚠️ Simulation completed with limited results")

    except Exception as e:
        logging.critical(f"💥 Critical simulation failure: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)